    ...
    public function __construct(array $data = [])
    {
        ...
        foreach ($data as $k => $v) {
            if (!array_key_exists($k, $this->data)) {
                throw new Exception('There should no be such field (' . $k . ') in ' . static::class . ' according to 1CClientBankExchange v1.03 format See http://v8.1c.ru/edi/edi_stnd/100/101.htm');
            }
            $this->data[$k] = $v;
        }
    }
